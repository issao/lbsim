...
**Last updated:** 2026-09-06 20:50 PDT by Claude.
...
| **Deployed, public at <https://lbsim.ai>** since 16:16, also <https://lbsim-irpwc2yaoa-uc.a.run.app>. **Since 20:39 the dashboard drives a real simulation live: revision `lbsim-00006-7sk` from `master` c90bf35** (U22 preemption, U28 web on the live transport, U52 mock tags), deployed by the main agent (creation time from `gcloud run revisions describe`), live path verified end to end on the public URL at 20:40: `POST /v1/ingress/StartRun` with `scenarios/route_p2c.txt` at 4x realtime returned `r-1`, `GetRun` `STATE_RUNNING`, `OpenSubscription` streamed SSE updates (a fleet row with 13 metrics and windowed distributions), `StopRun` finalised `STATE_COMPLETE`; <https://lbsim.ai/#/dashboard> is the live surface. Replay stays: <https://lbsim.ai/runs/index.json> serves the recorded demo runs (200 at 20:40); reports at `/reports/`. Previous revision `lbsim-00005-dzk` (image `:6897543`, 17:14) was the replay-only deploy; the build trend and the idle note are in `docs/deploy.md` (c706671) | `Dockerfile`, `cloudbuild.yaml`, `deploy.sh` | `curl -s https://lbsim.ai/runs/index.json`; `./deploy.sh --check-idle` reads the instance count from Cloud Monitoring |
...
**It does, since 17:20 PDT, and since 20:39 it also runs them live**: revision `lbsim-00006-7sk` from c90bf35 carries U28, so <https://lbsim.ai/#/dashboard> starts a scenario through the Ingress server and streams it over SSE, verified end to end on the public URL by the main agent at 20:40 (StartRun, GetRun, OpenSubscription, StopRun); the "live and showcased" count still waits on U49 for the showcase half. Before that: <https://lbsim.ai/runs/index.json> serves the 30 recorded demo runs and
