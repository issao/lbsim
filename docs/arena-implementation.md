...
## 6. The generator loop, as built

U34, in `crates/sim-arena/src/generator.rs`. What section 6 of `docs/arena.md` asks for is a
generator that writes policy *code*; what is built is the loop around such a generator, with a
template standing in for the forecasting families (U40, not built). The three variants it can render
today are deliberately plain: `p2c_d3` and `p2c_d4` are p2c with `d` fixed, and `least_kv_p2c` is p2c
keyed on `kv_tokens` instead of `queued_tokens`. Every step a smarter generator will need is exercised
end to end, and only the template has to change.

**The loop is two processes, not one.** The policy registry is generated by `sim-policy/build.rs`
from line 1 of every file in `crates/sim-policy/src/`, at build time. A name this process writes does
not resolve in this process. So `--keep` writes the file and rebuilds, then re-invokes the freshly
built binary with the internal `--score` flag, and that child is what runs the round and appends the
catalog row.

The exact sequence, from the workspace root:

```
# 1–2: render the source and print its path and sha256; nothing else happens
sim-run generate --family routing --variant p2c_d3 [--dir DIR]

# 1–5: same, but placed under crates/sim-policy/src/gen_p2c_d3.rs, then
#   3: tools/build.sh build --release -p sim-run          (registry now has the name)
#   4: tools/build.sh run   --release -p sim-run -- generate --score ...   (child: arena round on
#      scenarios/holdout, one policy, RoundConfig::default)
#   5: the child appends a CatalogRow to docs/policy-catalog.md
sim-run generate --family routing --variant p2c_d3 --keep --date 2026-09-06
```

Line 1 of the rendered file is exactly `//! lbsim-policy: routing names=<variant>`, the header
`build.rs` parses, and `tests/arena_generator.rs` restates that parser's rules rather than importing
the build script. The child refuses to score a file whose sha256 differs from what the template
renders, so a hand-edited `gen_*.rs` cannot be catalogued under the template's name. The catalog's
Source column is `<path>@<sha256[..12]>`; the date is `--date`, because simulation code reads no
clock; nothing in the loop draws randomness. SHA-256 is forty lines in `generator.rs` against known
vectors, because the workspace takes no dependency for it.

Measured once by hand, 2026-09-06, with the file and its catalog row then removed so no `gen_*.rs` is
committed and the catalog does not point at a file that is not there. The score is a real finding, not
a plumbing artefact: on the same suite and rule set the hand-written `p2c` (d=2) scores 0.762 worst
case and 0.872 mean, and `p2c_d3` scores 0.000 worst case (the gate closes on `h7-code-completion`)
and 0.746 mean. One more sample makes p2c *worse* under delayed telemetry, which is the herding effect
`p2c.rs` names as the reason it prefers a small `d`: the loop's first output is a variant the catalog
should record as tried and rejected.

```
crates/sim-policy/src/gen_p2c_d3.rs sha256 d60ea55f1d525ba9c2a1d79f2f2e83c0d15fbae3540101f8ce79006f5464c609
score p2c_d3 0.0000 worst h7-code-completion mean 0.7456 over 7 loads
catalog docs/policy-catalog.md += crates/sim-policy/src/gen_p2c_d3.rs@d60ea55f1d52
```

What is not here: any policy family beyond p2c variants (U40), a generator that proposes parameters
rather than picking from a fixed list, and scoring several variants in one round. The last is a
deliberate omission: one variant per `--keep` keeps the "which build scored this" question trivial.
