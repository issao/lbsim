...
**Last updated:** 2026-09-07 23:54 PDT. **RUNNING** (tech lead resumed 21:00 PDT on main's brief: Issao's three asks, machines page shows 0 replicas / slider to 10k / default 256 replicas, plus main's U94 GPU utilization and U95 partial-tag wording). Deployed: lbsim-00016-nbc from 3de8220; master is 6a7453b (proto: METRIC_GPU_UTILIZATION 67, METRIC_GPU_COMPUTE_BOUND_FRACTION 68). Housekeeping and productivity are not running; landings are recorded here only. **Spawned 21:00:** U90 (default, `claude/tl-machines-live`, port 8195), U91 (sonnet, `claude/tl-replicas-10k`, port 8196), U92 (default, `claude/tl-default-256`), U93 (sonnet, `claude/tl-qa-replay-wait`, port 8199), U94a (default, `claude/tl-gpu-engine`), U94c (default, `claude/tl-gpu-web`, port 8197), U95 (sonnet, `claude/tl-partial-tag`, port 8198). U94b (wire) spawns when U94a lands. **21:07:** U96 (default, `claude/tl-ab-live`, port 8200) spawned from main's A/B decision; U109 (lockstep A/B via StepForward) queued. **21:09:** master was red since 6a7453b (the proto's two GPU metrics were missing from `wire::METRICS`, so `metric_table_matches_the_proto_enum` refused every integration at stage 3); the tech lead's one-line unblock fix is fc6d310. U93 is done on its branch (6ce6f2a: lbsim.ai 57/2 before, 59/0 after) and re-integrating. **21:12:** U94a landed (20cb4d9); U94b (sonnet, `claude/tl-gpu-wire`) spawned. **21:13:** U93 landed (fb9972b), U90 landed (258c706), U91 landed (c9d7c0d). In flight: U92, U94b, U94c, U95, U96. **21:18:** U92 landed (ba009c7); before/after numbers per finding sent to main (finding 2, staleness, needs rewriting: at 256 replicas the herd is past the cliff at 100 ms). U94c is re-integrating after a two-line MachineLevel fix (U90's `pendingRow` lacked the new fields) with ownership of types.ts extended. In flight: U94b, U94c, U95, U96. **21:20:** U94c landed (64d9ad0), U95 landed (d01f991). Main's decisions on U92's numbers: demo 2 goes back to 32 replicas (**U92b**, sonnet, `claude/tl-staleness-32`, spawned 21:20); the cliff moving with fleet size is a finding of its own (**U98** queued, demo 13); a docs agent of main's rewrites findings.md. Main's load test on the release build: 10k replicas is 1.5–1.9× realtime on one core and the 120 s run trips `MAX_EVENTS` (**U97**, sonnet, `claude/tl-event-ceiling`, spawned 21:20); numbers recorded under M9 below. In flight: U94b, U96, U97, U92b. **21:21:** U96 landed (53c5993). In flight: U94b, U97, U92b. **21:32:** U92b landed (ff4e0ca: demo 2's rows byte-identical to pre-U92). U97 landed (2eb5d4f) with its ORIGINAL scope (scaled event ceiling + hardcoded 8 GiB RSS guard): the agent declined a mid-flight redirect relayed as a peer message, correctly, so Issao's design is **U97b** (sonnet, `claude/tl-memory-budget`, spawned 21:32): no event ceiling at all, `LBSIM_MEMORY_BUDGET_MB` (default 20000; the Dockerfile sets ~80 % of the container), in-flight/records/queue caps scaled with replicas × duration. **U99** (sonnet, `claude/tl-no-jump`, port 8203, spawned 21:32) from Issao's jumpy-cards note. **Standing rule from Issao (21:32): two dynamics lanes are always in flight**, working through docs/scope-today.md beyond package B in the graph's order (prefix caching 9/10, tiering 14, disaggregation 15, autoscaling 12, multi-geo 13, weight locality 17, control analysis M7, failover cascades); each lane ends its unit the way the demos do (scenario, report, recording, walkthrough, finding paragraph to main) and starts the next; dashboard/polish units run beside them. Lanes now: **lane A = U98** (default, `claude/tl-herd-fleet`, port 8201, demo 13), **lane B = U27a** (default, `claude/tl-prefix-engine`) with **U27b** (default, `claude/tl-prefix-policy`, port 8202, demo 14) coding against U27a's seam in parallel; U27c (wire + report + web keys) queued behind U27b. **U29 (Leaf shards as processes, M3) is future work by Issao's decision** ("let's not do sharding then, leave it recorded for future work"): the single-threaded core does 10k replicas at 1.5–1.9× on one core and the 5×10k-GPU target is 6,250 replicas, so sharding is the lever only past that; the U15 Leaf seam stays as the boundary it would plug into. In flight: U94b, U97b, U99, U98, U27a, U27b. **21:35:** Issao asked again what is mock about a live run → **U95b** (default, `claude/tl-no-invented`, port 8204): no invented number on live/replay, unwired cells render "—" with the hover "not simulated yet", tags carry the mode word only; **U101** (sonnet, `claude/tl-replica-state`): replica state, true speed and a per-replica TTFT mean on the wire (proto 69/70 landed by main at 21de1ba). Then Issao, verbatim: *"yeah, let's remove all invented numbers everywhere, every reference to mock in the ui, every link for a mock data view. it is not needed anymore."* → **U100** (default, web) queued to spawn when U95b and U99 land so the last web unit of the wave rebases the others rather than the reverse: the browser mock engine and everything that exists only for it goes; data modes are live and replay only; **the stand-in dashboard's role ended today.** deploy.sh already sets `LBSIM_MEMORY_BUDGET_MB=1600` for the 2 GiB container (661cc65). In flight: U94b, U97b, U99, U98, U27a, U27b, U95b, U101. **21:35:** **U102** (play button on the walkthrough card) queued behind U100. **21:35:** U94b landed (9d4aa1b): GPU utilization and the KV distribution are on the wire and in the export; only `summary_md5` moved. U94 is complete end to end; qa.js's two non-zero-mean checks should now pass (verified in the wave's final harness run). In flight: U97b, U99, U98, U27a, U27b, U95b, U101. **21:38:** **U103** (draggable walkthrough card) queued with U102. **21:39:** from main's vision-progress refresh: **U104** (real traces on the Traces tab, after U100) and **U105** (rewind from the checkpoint, after the lanes' current units) queued; **U31b** (outlier ejection / failover) is lane A's next unit after U98, because it unlocks three later items and needs nothing. Order of the remaining web units: U95b, U99 → U100 → U102, U103, U104. **21:45:** master was red a second time (main's d32f079 added 69/70 to the proto without the wire table); the tech lead's one-line fix is 2ba17f2. Done on their branches and re-integrating: **U98** (59994da: demo 13's five points, SLO 79.5 → 56.5 → 29.5 → 14.5 → 7.0 % from 32 to 512 replicas at 0.30 ratio and 250 ms; finding 10 paragraph in its report, forwarded to main with the READY), **U27a** (d6e1978: the seam as specified, `PrefixTree` lives in sim-workload and is re-exported by sim-model), **U97b** (b867135: no event ceiling, `LBSIM_MEMORY_BUDGET_MB` default 20000, caps scaled; the 10k-replica 120 s run completes locally in 71.5 s wall at 916 MB peak RSS). Both U98 and U27a edited `tests/scenario_parse.rs`'s exhaustive literal and KEYS count, so the second to integrate resolves that conflict. **21:48:** U95b landed (e88b73c); U100 spawns when U99 lands. Issao: *"where do i tune step token budget?"* → **U106** (editable physics knobs with a Restart path) queued after U100. **21:51:** U98 landed (d94fbf0), lane A moves to **U31b** (default, `claude/tl-ejection`, port 8205, demo 15). In flight: U99, U27a (re-integrating), U27b, U97b (re-integrating), U101, U31b. **21:54:** U27a landed (361d59a; U27b told to rebase); U97b landed (925f758: no event ceiling, `LBSIM_MEMORY_BUDGET_MB`, scaled caps; the 10k-replica 120 s run completes in 71.5 s wall at 916 MB peak). Issao: *"the scrolling of control seems wrong ... In general the full screen shouldn't scroll, just individual panels."* → **U107** (default, `claude/tl-app-shell`, port 8206) spawned beside U99. /tmp (16 GB tmpfs) filled to 97 % with agents' export scratch and made one integrate report a false "fingerprints moved"; cleaned to 45 %. In flight: U99, U27b, U101, U31b, U107. **21:56:** Issao: *"I assumed policies would cover the routing decision, the host and gpu local scheduling decision and the global admission/rejection decisions. is that not the case?"* → **U108** `SchedulingPolicy` queued as the next lane unit (whichever of U31b / U27b–U27c frees first), ahead of U30 tiering and U32 autoscaling, which are written against it. **22:00:** U99 landed (685f138) and U101 landed (25b414c); **U100** (default, `claude/tl-no-mock`, port 8207) spawned: the mock goes, U101's three fields map into the replica table, and the replicas table reserves its rows so U99's showcase-card height check (failing on master because the table grows from 0 to pageSize rows as samples stream) turns green. **U110** (sonnet, `claude/tl-tmp-hygiene`): `tests/wire_export.rs`'s demos test had left 1,022 scratch dirs (7.7 GB) on the shared tmpfs; it cleans up after itself. In flight: U27b, U31b, U107, U100, U110. **22:04:** U27b landed (485f368, demo 14, 16 cards / 14 reports / 50 runs); lane B moves to **U108** (default, `claude/tl-scheduling`, port 8208, demo 16) with **U27c** (sonnet, `claude/tl-prefix-wire`) closing the prefix work beside it. In flight: U31b, U107, U100, U110, U108, U27c. **22:05:** U110 landed narrow (4891264: wire_export's guard only; the agent rightly declined a mid-flight widening delivered as a message), so the widened mandate is **U110b** (sonnet, `claude/tl-scratch-guard`) with the ownership granted in its brief. Lesson recorded: widen by spawning, never by message. In flight: U31b, U107, U100, U108, U27c, U110b. **22:07:** U107 landed (fa42706). In flight: U31b (harness), U100, U108, U27c, U110b. **22:18:** U31b landed (02039ea, demo 15); lane A moves to **U27d** (default, `claude/tl-cascade`, port 8209, demo 17: the affinity failover cascade, dynamic 10, no new engine seam). In flight: U100, U108, U27c, U110b, U27d. **22:30:** U100 landed (dcc226d): **the stand-in dashboard's role ended today**; data modes are live and replay only. Spawned against the mock-free tree: **U102** (default, `claude/tl-wt-play`, port 8210), **U104** (default, `claude/tl-real-traces`, port 8211), **U106** (default, `claude/tl-physics-knobs`, port 8212); U103 follows U102. In flight: U108 (QA), U27c, U110b (dry run), U27d, U102, U104, U106. **22:33:** U27d landed (8b2348f, demo 17). Two facts it found → **U111** (sonnet, `claude/tl-ready-count`): the fleet's ready count stayed 256 through a crash (`METRIC_READY_REPLICAS` = fleet size, not `state != 3`), and Home's report list has a nested `<li>` from concurrent appends. Lane A's next: **U33** (default, `claude/tl-bode`, port 8213, spawned 22:37): keys `perturbation = sine`, `perturb_amplitude`, `perturb_frequency_hz`; demo 18 sweeps the frequency at demo 2's 32-replica fleet with `--telemetry`; `bench/bode.py` (stdlib) computes gain/phase/harmonic content and writes the Bode plot, predicting the 180° crossing at `1 / (2 (D + I/2))` = 0.71 Hz for 200 ms delay and 1 s interval; walkthrough `stale-oscillation.json`. In flight: U108, U27c, U110b, U102, U104, U106, U111, U33. **22:41:** U106 landed (30065da). In flight: U108 (cargo chain), U27c, U110b, U102, U104, U111, U33. **22:48:** U108 landed (ce4a0bb, demo 16); lane B moves to **U30** (default, `claude/tl-tiering`, port 8214, demo 19: cluster DRAM/SSD pools and a shared fabric container, written against U108's victim seam). Two U108 decisions sent to main to ratify: the trait lives in sim-core (layering forbids either direction between sim-model and sim-policy) and a fourth decision point `prefill_order` (default None), because at ≤1.5× load 256 replicas at batch 256 never queue. In flight: U27c, U110b, U102, U104, U111, U33, U30. **22:56:** U111 landed (8d956c9: ready count excludes crashed replicas; Home's list flat), U104 landed (e2fa65b: real traces, `GetTraces` implemented). Main ratified U108's two decisions (the ARCHITECTURE 10.8 note and the catalog rows go to main's docs agent). In flight: U27c, U110b, U102, U33 (harness), U30. **22:59:** U102 landed (d1395f3) with a real finding: over HTTP/1.1 (the local server) the Machines tab's ten replica SSE streams plus the fleet stream exceed Chrome's six-per-host limit and starve every later RPC (SetSpeed, GetRun, Renew), so the walkthrough's pause sometimes never reaches the server; lbsim.ai is HTTP/2 and unaffected. → **U112** (default, `claude/tl-sse-cap`, port 8215): a stream budget (4 under HTTP/1.1, 32 under h2) with time-multiplexed replica rows. **U103** (sonnet, `claude/tl-wt-drag`, port 8216) spawned now that U102 is on master. In flight: U27c, U110b, U33, U30, U112, U103. **23:00:** U27c landed (1ee2f80: hit rate on the wire and in the report when `prefix_roots > 0`; measured 18 / 20 / 24 % for p2c / spread / sticky). It found that `api.ts`'s `SCENARIO_KEYS` lacks the prefix keys and `ROUTING_TO_ENGINE` nulls `prefix_affinity`, so the affinity card's live run silently ran p2c without a prefix model → **U113** (sonnet, `claude/tl-wire-keys`, port 8217): the web's key tables track `Scenario::parse` with a drift self-test. In flight: U110b, U33, U30, U112, U103, U113. **23:09:** U33 landed (32859aa, demo 18: the loop rolls off like an integrator, phase plateaus near −100°, never reaches the delay-only 180° prediction at 0.71 Hz; it self-oscillates at 0.02–0.025 Hz, a 40–50 s period set by request residency, and the input becomes a ripple on the herd). Lane A moves to **U32** (default, `claude/tl-autoscale`, port 8218, demo 20: an `AutoscalingPolicy` seam, WARMING/DRAINING/ABSENT lifecycle, cold start 30 s vs 5 s under U33's diurnal sine; multi-geo stays queued). In flight: U110b, U30, U112, U103, U113, U32. **23:14:** U112 landed (19d0e73); U102's three checks pass three runs in a row locally. In flight: U110b, U30, U103, U113, U32. **23:19:** main's interim deploy lbsim-00017-2wq (from d5da849, harness 82/12) found two things → **U114** (sonnet, `claude/tl-report-size`): at 256 replicas the reports are up to 29 MB and two answer HTTP 500 on Cloud Run (32 MiB non-chunked cap); every report under 8 MB. Issao, verbatim: *"in utilization, it is not showing any preemptions to kv cache even when it looks like we are max and they should be happening, something is missing."* → **U115** (default, `claude/tl-preempt-wire`, port 8219): metrics 46/47 were never served, and the dashboard's default sends no `preemption` key so the engine runs `never`; serve both, default the dashboard to `swap` with a DRAM pool, state the mode on the KV panel, a restart-to-apply knob; demo 5 under swap measured for Issao's choice of the file default. In flight: U110b, U30, U103, U113, U32, U114, U115. **23:31:** U110b landed (5f90db6) and its gate immediately refused U30 twice ("grew /tmp by 1408 MB") for other agents' exports on the shared tmpfs: the tech lead's **U110c** fix (`claude/tl-gate-tmpdir`) runs stages 3–4 under a private `TMPDIR` (the guards resolve `temp_dir()`) and measures only that. U30 is done on `claude/tl-tiering` (599350e) and re-integrates when the fix lands. In flight: U30 (blocked on U110c), U103, U113, U32, U114, U115, U110c. **23:37:** U110c landed (a670ab9; the gate left 1 MB on its own run); U30 told to rerun. U113 landed (5252ff4: 73 keys, `prefix_affinity` sent, a drift self-test against the parser); it found the two affinity walkthroughs never set the prefix keys, so their live runs had no prefix model → the tech lead's one-commit fix **U113b** (`claude/tl-wt-prefix-keys`, five keys in both scripts) is in the merge queue. In flight: U30 (re-integrating), U103, U32, U114, U115, U113b. **23:45:** U113b landed (77b2ec9), U115 landed (9cd8751). In flight: U30 (re-integrating), U103, U32 (integrating), U114 (integrating). READY gates left: U103, U114. **23:51:** U114 landed (173ce85: 29 MB → 7.5 MB, every report under 8 MB), U103 landed (987683b). Every READY gate is on master; the tech lead's full harness run (with reports) against 8da5986 is in progress in a fresh worktree. In flight: U30 (re-integrating), U32 (integrating). **23:54:** the harness against 8da5986 with reports: **`qa: 100 passed, 0 failed`** (`/tmp/tl-qa-final.log`; U103's own checks passed on its rebase onto 8da5986). **READY TO DEPLOY 987683b sent to main** with the collected decisions (proto wording for 27/28 and the 69 state values, the trace budget, U31b's ejection default, the stand-in's end in the docs, the widen-by-spawning lesson). Two code comments that began with `Issao:` to cite the ask were read by the inbox scanner as pending instructions; reworded (`claude/tl-inbox-quotes`). Waiting only for U30 and U32 to land, then STOPPED. READY TO DEPLOY goes to main when U90–U96 are all on master and tools/qa/qa.js is green locally. **Done 115 · in flight 2 · queued 11 · waiting on Issao 1 · future work 1 (U29).**
...
**Done on its branch 23:31** (599350e; blocked on U110c's gate fix): keys `dram_pool_tokens`, `ssd_pool_tokens`, `ssd_gbps`, `fabric_gbps`; `sim_model::{Tier, Tiers}` with one `fabric_busy_until` container; 95 golden rows byte-identical before demo 19; 27/28 at fleet scope as a value plus a Distribution whose percentile slots are tier ids (1 DRAM, 2 SSD) — **for main's proto wording**; `api.ts` `EXTRA_KEYS` +4 (flagged). Kept 4 replicas (the spiral's fleet). Numbers: tier_dram 1,421 tok/s goodput / 100 % / TTFT p99 233 ms / ITL p99 70 ms vs swap-everything 1,376 / 97.6 % / 84 / 82; tier_dram_ssd (10 GB/s SSD): goodput 893, 70.3 %, ITL p99 361 ms — a tier slower than the ITL budget is worse than none; tier_contended (`fabric_gbps = 10`): goodput 666, 59.6 %, TTFT p99 4.2 s, barely above never-swap's 608: the swap advantage inverted. Finding paragraph in its report.
**Spawned 22:48** on `claude/tl-tiering`, port 8214 (lane B): keys `dram_pool_tokens`, `ssd_pool_tokens`, `ssd_gbps`, `fabric_gbps` (defaults = today's per-replica DRAM path, proven by `kv_spiral_swap`'s golden row); cluster `TierPools` and a `Fabric` bandwidth container that serialises migrations; `Frame` tier gauges; wire 27/28 at fleet scope; demo 19 `tier_dram` / `tier_dram_ssd` / `tier_contended`; walkthrough `kv-tiering.json`. Disaggregation (15) stays queued as U30b/U30c after it.
...
**Spawned 23:09** on `claude/tl-autoscale`, port 8218 (lane A), the single-cluster half: `AutoscalingPolicy { desired(&FleetView) }` (`none` | `target_utilization` with hysteresis, cooldown, step), keys `autoscaling`, `autoscale_target/interval_s/step/cooldown_s`, `min_replicas`, `max_replicas`, `warmup_delay_s`, `drain_timeout_s`; slots `0..max_replicas` with ABSENT (0) / WARMING (4) / DRAINING (5) states beside 1/2/3; wire 60/61/62 from state; demo 20 `autoscale_none` / `cold30` / `cold5` under a 200 s diurnal sine; walkthrough `autoscaling.json`. Multi-geo (13) stays queued.
Replica lifecycle with turn-up delay, warm pools, diurnal load per cluster, cascading failure. Upstream U31.

### U33 M7 control analysis (lane A, default model)
**Landed 23:09** as 32859aa: keys `perturbation`/`perturb_amplitude`/`perturb_frequency_hz` (91 rows unchanged before the demo); demo 18 with `--telemetry`; `bench/bode.py` (stdlib) writes `out/18-bode-plot.html`. Output is `fleet_running`, not `fleet_queue` (at this fleet arrivals go straight into batches). Gain 1.59 (+4 dB, −39°) at 0.01 Hz → 0.014 (−37 dB, −103°) at 1 Hz; harmonic content 0.3 → 35; dominant frequency 0.02–0.025 Hz from 0.1 Hz up. Finding paragraph in its report (forwarded with the READY). The perturbation keys need U113's key tables to be settable from a walkthrough.
**Spawned 22:37** on `claude/tl-bode`, port 8213, demo 18; see the status line. Perturbation input, frequency sweep, empirical Bode plot predicting oscillation onset. Upstream U15 (U32 is not needed for the single-cluster loop).
...
**Landed 22:00** as 685f138: `Tile` always renders its flag and note rows at fixed heights (`visibility: hidden` when empty), `.panel-head` min-height, the banner folds the refusal message into the run-id line, the status bar is one nowrap line, `data-tile="wasted"` on the named tile; qa.js section h (three checks) and `screens.js` `QA_STABILITY=1`. Its showcase-card check still fails on master: the replicas table grows from 0 to pageSize rows as the first samples stream (a row-count change, not text height); U100 reserves the rows.
...
**Landed 22:18** as 02039ea. Demo 15 runs p2c (least_requests is already at 4.5 % SLO on this fleet with no failure, identical in both arms): none vs eject SLO 97.1 / 97.1 %, TTFT p99 3355 / 3322 ms, goodput 143,739 / 143,791; at the replica: 49 requests hit the gray replica after onset without ejection (48 missed SLO) vs 20 with; detection latency 4.3 s (ejected 64.3 s, re-admitted 94.3 s, re-ejected 100.3 s). The fleet aggregate barely moves at 256 replicas; the per-replica view is where the finding lives. Deviations: `ejection_ratio = 8` in the demo because a healthy fleet's step time is bimodal (11 ms decode vs 48 ms with a prefill chunk, 4.4×) and ratio 3 ejected 146 healthy replicas by t=20 s; the Scenario default stays 3.0 (**decision for main**: follow the demo to 8, or keep 3 and document). `crates/sim-policy/build.rs` gained the `health` kind (the registry is generated); `Sim::policy_views()` added because neither `latest_views()` nor the export carries a policy ejection.
...
### U27d affinity hotspot failover cascade; demo 17 (lane A, default model)
**Landed 22:33** as 8b2348f. Hot replica 0 (holder band 0–5). At the crash the holder had 36 running / 145k KV tokens vs p2c's 8 / 39k; 73 running timeouts vs 33; inheriting replicas 2/156/46 queue peaks 9/5/2 vs 1; replica 156's step 15.7 → 28.7 ms; the returning replica is at 90 % load within 0.5 s and runs 27 % slower for ~15 s. Fleet aggregate barely moves (96.2 % vs 96.4 % without the crash): the cascade is a per-replica event at this scale. Finding paragraph in its report (forwarded with the READY).
**Spawned 22:18** on `claude/tl-cascade`, port 8209. Dynamic 10 (needs 9 and 4, both landed): `cascade_affinity.txt` (sticky affinity + crash of the measured hottest holder at t=60, back at 90) against `cascade_p2c.txt`; the inheriting replicas' queue and TTFT spike, the returning replica is cold; walkthrough `affinity-cascade.json`; finding paragraph to main. No Rust change expected.
...
**Landed 22:04** as 485f368. Three runs at 256 replicas / 560 rps, 100 roots × 800 tokens, Zipf 1.0, fork 0.1, 100 ms telemetry: p2c goodput 147,043 tok/s, SLO 98.7 %, CV 0.32, TTFT p99 2,282 ms; spread (1.05) 145,303 / 97.8 % / 0.37 / 3,221; sticky (2.2) 141,314 / 96.4 % / 0.75 / 3,959; ratio 4.0 → CV 1.36. Affinity never pays here: a hit saves an 800-token prefill (~28 ms) in a decode-bound run. Deviation flagged: the scenarios use 100 ms / 10 ms telemetry, because at the baseline's 1 s / 200 ms every ratio herds to CV ≈ 1.2 (dynamic 2's cliff through another policy). Finding paragraph in its report (forwarded with the READY).
...
**Landed 23:00** as 1ee2f80: metric 48 at both scopes, gated on `prefix_roots > 0` (the engine's `prompt_total` counts every admission, so an ungated ratio would read a meaningless 0 % everywhere); report rows gated the same; only demo 14's and 17's md5 columns moved. Measured hit rates: p2c 18 %, spread 20 %, sticky 24 %. Left for U113: `api.ts`'s key tables.
**Spawned 22:04** on `claude/tl-prefix-wire`; demo 14's four md5 rows move (conditional report rows), nothing else.
...
**Landed 22:00** as 25b414c: `Replica::state()`, TTFT counters, `ReplicaSample.state/speed/ttft_sum_ns/ttft_count`, 69/70 at replica scope and the fleet mean speed, per-replica `METRIC_TTFT` as a count+mean Distribution; two rebases (U94b, U27a) resolved keeping both sides. Also swept 7.7 GB of stale export scratch from /tmp (U110 makes the test clean up).
...
**Landed 22:30** as dcc226d. Deleted: `engine.ts` (MockEngine), `rng.ts`, `traces.ts`, `subscriptions.ts`, `useSubscriptions.ts`, `Waterfall.tsx`; types moved to `frame.ts`; removed charts: tier occupancy, tier bandwidth, wasted-GPU, failure events, warming/draining, preemptions series; `MockTag`, `Panel.data`, `realness`, the status bar's registry counters, Home's Mock section, `.mock-*` CSS; badge is `.mode-badge`; `dataModeFrom` answers `none` → "no server and no recordings served". U101's fields mapped (state, speed, TTFT mean) and both subscription lists request 67/68/69/70/TTFT. Harness 69 passed, 15 report 404s under QA_SKIP_REPORTS. Left: `config.ts BASE` still says 70 rps against base.txt's 560 (two self-tests fail on master since U92; U106 re-pins them).
**Spawned 22:00** on `claude/tl-no-mock`, port 8207, with the full mock inventory (35 files) in its brief; U107 rebases against it.
...
**Landed 22:59** as d1395f3: `WalkthroughRunner.resume()`/`pauseHere()`, `paused` in `StepState`, one `.wt-play` button, the body reads "advancing scenario… to T s at N×" / "paused at C s; play to continue to T s"; the playback bar reads a `PlaybackOverrideContext` the Showcase provides. Harness locally: pause and play pass; `bar play is the same action` could not be exercised under HTTP/1.1 stream starvation (U112).
**Spawned 22:30** on `claude/tl-wt-play`, port 8210 (default model: one runner action `resume()`/`pauseHere()`, two buttons).
...
**Landed 23:51** as 987683b: `useDraggable` (pointer events, clamped, localStorage in try/catch, reset in the header); three harness checks pass. Its first integrate was refused by U110b's whole-disk gate for other agents' scratch (455 MB); the rerun on U110c's gate was clean.
**Spawned 22:59** on `claude/tl-wt-drag`, port 8216: `useDraggable` over pointer events, clamped, remembered in localStorage, a reset affordance in the header.
...
### U114 every report under 8 MB (`model: sonnet`)
**Landed 23:51** as 173ce85: heatmap capped at 64 replicas (32 most and least loaded) and 250 columns, line charts at 2,000 points; the 4-run compare 29.1 → 7.5 MB, 18-bode 14.9 → 3.9 MB, largest now 6.9 MB; `tests/report_size.rs`; only html md5 columns moved.
**Spawned 23:19** on `claude/tl-report-size`. Per-replica tables capped at 64 (32 most and least loaded), series ≤ 2,000 points per line, telemetry never embedded; `tests/report_size.rs` asserts the 256-replica routing compare's size; only html md5 columns move.

### U115 preemptions and retries on the wire; the dashboard defaults to `preemption = swap` (default model)
**Landed 23:45** as 9cd8751: `Frame.retries`, `ReplicaSample.preemptions` (window deltas); 46/47 served; dashboard default `extra = { preemption: 'swap_to_dram', preemption_victim: 'newest', dram_capacity_tokens: 5480000 }`; the KV panel states mode and rate; Cluster tab `Select`s staged through the restart path; `base.txt` unchanged. Demo 5 under swap is byte-identical to `never` at every point (the batch limit binds before the 1.37 M-token cache), so the file default stays. Left: `retriesPerS` in the web `Frame` type (47 served, unread).
**Spawned 23:19** on `claude/tl-preempt-wire`, port 8219. `METRIC_PREEMPTIONS_PER_S` at fleet and replica scope, `METRIC_RETRIES_PER_S` at fleet scope from a new window counter; `BASE.extra` sends `preemption = swap`, `preemption_victim = newest`, `dram_capacity_tokens` = 4× KV; the KV panel says "preemption: swap (newest victim) · N/s" (or explains `never`); Cluster tab knobs staged through U106's restart path; `scenarios/base.txt` untouched; demo 5 under swap reported for Issao.

### U113 the web's wire key tables track `Scenario::parse` (`model: sonnet`)
**Landed 23:37** as 5252ff4: `EXTRA_KEYS` 58 → 73 (every parser arm, incl. U33's three), `ROUTING_TO_ENGINE.prefix_affinity` real, `policiesToWire` sends the affinity knobs with that kind, a drift self-test that reads the parser and a round-trip test over every extra key, the affinity card in `REQUIRED_KEYS`. Found: `affinity-vs-spread.json`/`affinity-cascade.json` never set the prefix keys (U113b, the tech lead's fix).
**Spawned 23:00** on `claude/tl-wire-keys`, port 8217. `EXTRA_KEYS`/`SCENARIO_KEYS` list every parser key (prefix, ejection, scheduling, perturbation, tiering as landed), `ROUTING_TO_ENGINE` maps `prefix_affinity`, a self-test reads the parser and fails on drift, `REQUIRED_KEYS` gains the affinity card.

### U112 the dashboard never starves its own RPCs: a stream budget under HTTP/1.1 (default model)
**Landed 23:14** as 19d0e73: `streamBudget()` 4 / 32; every stream counted; `ReplicaStreams` rotates one slot per `2 s / slots` so a 10-row page is covered in 12 ticks; status bar `streams N/B`; the Machines pager says "N at a time" when rotating. U102's checks pass 3/3 locally; `machines()` waits for the tab.
**Spawned 22:59** on `claude/tl-sse-cap`, port 8215. From U102's finding. `streamBudget()` = 4 when `nextHopProtocol` is not h2/h3, else 32; `useServerReplicas` opens at most budget − 1 replica streams and round-robins the visible page every 2 s; the status bar shows streams N/B; self-test with a counting fake stream; U102's three checks must pass twice locally.

### U104 real traces on the Traces tab; GetTraces implemented (default model, after U100)
**Landed 22:56** as e2fa65b. `GetTraces` answers at any run state from a ring the run thread fills via `Sim::drain_traces()` (newest 2,000 or 5 MiB encoded), filters per the proto (outcome name or number, `min_e2e_ns`, `tenant_id`, `limit` default 100), newest first by the instant the request ended; `record_traces` sets a 5 % sample when the scenario has none; the demos export samples the same; fingerprints unchanged. Harness: 23 rows, 256 ready, no invented numbers. **Decision for main:** each encoded trace is ~570 KB (one decode span per output token with the full resource state), so the 5 MiB ring holds 9–25 journeys and the demos export kept 9 of 943; coalescing decode spans or raising the budget is a proto/wire decision.
**Spawned 22:30** on `claude/tl-real-traces`, port 8211: `Sim::drain_traces()`, `RunState.traces` bounded, `GetTraces` with the proto's filters, `record_traces` sets a 5 % sample when the scenario has none, the demos export records at 5 % for replay, a fresh `Waterfall`.
...
**Landed 22:41** as 30065da: structural paths (`seed`, `durationS`, `warmupS`, `fleet.*`) stage `pendingRestart`/`pendingKeys` with no RPC; the banner offers Restart (`#restart-pending`); the Cluster tab has all five physics sliders (two were absent entirely); readouts on replay. Harness: r-1 → r-2 with `step_token_budget = 2048` sent. Self-tests re-pinned to 560 rps (api 26/29, server b), server (e) re-pinned to staging, (n) new; the web self-tests are green on master again.
**Spawned 22:30** on `claude/tl-physics-knobs`, port 8212; also re-pins `config.ts BASE` to base.txt's 256 / 560 and the two self-tests.
Issao asked, verbatim, *"where do i tune step token budget?"* Main: the Cluster tab's physics knobs (step token budget, prefill rate, KV per replica, max queue, accelerator, replicas) are greyed "server" and inert because UpdateWorkload/UpdatePolicies take only workload/policy keys and there is no restart path. On live they become editable; a change shows "restart the run to apply: <keys>" with a Restart button that sends StopRun + StartRun with the edited config at the same seed and reopens the subscription (`restart(next)` on `RunHandle` is the seam); workload/policy knobs keep applying live; on replay everything stays locked as "recording". Each physics knob gets a one-line explanation in docs/calibration.md's words. Harness: change the step token budget on a live run, assert the banner, press Restart, assert a new run id and the new value in the run's scenario.

### U107 app-shell layout: the page never scrolls, panels do (default model, with U99)
**Landed 22:07** as fa42706: `shell` class on `<html>` for every route but Home; html/body/.page `overflow: hidden` at 100dvh; each panel body its own scroll container; A/B one scroll container for both sides so rows align; the showcase card list scrolls under the header. Harness section i at 1280×720 (at 1400×900 the Cluster tab fits and the checks pass vacuously): control panel `scrollTop 209, scrollY 0`, status bar 614 ≤ 698 px. Screenshots at both sizes.
...
### U108 `SchedulingPolicy`: replica-scope scheduling as a pluggable policy (next lane unit, default model)
**Landed 22:48** as ce4a0bb; **spawned 22:04** on `claude/tl-scheduling`, port 8208, as lane B. `SeqView`/`StepView`/`SchedulingPolicy { admit_order, prefill_budget, victim }`, views bounded to `max_batch` queue entries; `fifo_chunked` proven byte-identical before demo 16 is added; `class_priority`, `deadline_first`; key `scheduling`; arena rows flip; demo 16 under `slo_classes = interactive:0.7,agent:0.2,batch:0.1`; `ClassPriority class_priority = 14` is on master (d9a2757).
Issao, verbatim: *"I assumed policies would cover the routing decision, the host and gpu local scheduling decision and the global admission/rejection decisions. is that not the case?"* Today only `RoutingPolicy` and `AdmissionPolicy` are seams; replica scheduling is fixed engine logic behind knobs. Main: a `SchedulingPolicy` trait in sim-policy at replica scope, one file per policy, generated registry, deciding per step from a view of the queued and running sequences (class, deadline, context size): (a) which queued sequences join the batch and in what order, (b) the prefill chunk budget for the step, (c) the preemption victim when the KV budget is exceeded. The engine's current behaviour becomes `fifo_chunked` (fifo, fixed `step_token_budget`, U22's victim rule), proven byte-identical by the fingerprints; then `class_priority` (U25a's classes: interactive before batch, batch preempted first) and `deadline_first` (earliest slack first). Key `scheduling = <name>`; `PolicySpec.replica_scheduling = 3` (`SchedulingPolicy`) already exists in policy.proto (its body checked by the tech lead, reported to main); the arena scores it like the others and the catalog rows flip from idea to shipped as they land; demo 16 contrasts the three under the mixed-class workload; finding paragraph to main. Cost per decision O(batch) at most, with a test that says so. Before U30 and U32.

### U111 the fleet's ready count excludes crashed replicas; Home's report list is flat (`model: sonnet`)
**Landed 22:56** as 8d956c9: `METRIC_READY_REPLICAS` counts `state != 3` in run.rs and export.rs (the export used the scenario's replica count); the fleet mean speed excludes crashed replicas; a test crashes replica 0 mid-run. Home: 17 flat `<li>`.

### U110 every test cleans up its scratch; integrate.sh refuses a gate that grows /tmp (`model: sonnet`)
**Landed 22:05** as 4891264, wire_export only; the rest is U110b.

### U110b the shared scratch guard in every test family; integrate.sh exit 8 on /tmp growth (`model: sonnet`)
**Landed** as 5f90db6. Its whole-disk measure was wrong on a shared tmpfs (U110c).

### U110c the scratch gate measures a private TMPDIR (tech lead, one-file fix)
**Landed 23:37** as a670ab9.
**Spawned 23:31** as the tech lead's own commit on `claude/tl-gate-tmpdir`: `mktemp -d /tmp/lbsim-gate-XXXXXX`, `TMPDIR` for stages 3–4, `du -sm` of it, removed after.
**Spawned 22:05** on `claude/tl-scratch-guard`: `tests/common::scratch()` guard (kept on panic), private copies in sim-ingress and sim-arena, every ad-hoc scratch replaced, `tools/integrate.sh` measures /tmp across stages 3–4 and refuses above 100 MB.
**Spawned 22:00** on `claude/tl-tmp-hygiene`. `tests/wire_export.rs`'s `fresh_dir` returns a guard that removes the directory on drop (kept on panic for inspection) and sweeps siblings older than an hour; measured cause of tonight's ENOSPC false "fingerprints moved". **Widened 22:01** per main: 3,582 lbsim scratch dirs (7.5 GB) from five test families, not one; one shared guard in `tests/common/mod.rs` (a private ten-line copy inside sim-ingress and sim-arena), every ad-hoc scratch replaced, and `tools/integrate.sh` refuses (exit 8) when the gate grows /tmp by more than 100 MB. Proto: `ClassPriority class_priority = 14` is on master (d9a2757) for U108.

### U109 lockstep A/B through StepForward (queued, default model)
