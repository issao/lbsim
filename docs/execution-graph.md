...
**Last updated:** 2026-09-07 19:52 PDT. **RESUMED 19:18 PDT** (Issao: "resume"). A fresh tech lead resumed from the PAUSED entry below. **19:18:** U80 landed (023672d: Home.tsx in three `h2` sections Live / Replay / Mock, the only file; integrated by the tech lead per main's instruction, gate green). Re-spawned in place from their branches: **U81** (sonnet, `claude/tl-mode-badge` from da04703, port 8193), **U82** (sonnet, `claude/tl-finished-text`, port 8194; its six edits listed in its section), **U79** (default, `claude/tl-cloud-hang` from bae16cc, port 8191). Decision: **U77 is no longer folded into U79**; it is its own unit spawned when U79 lands, because both edit `drive()`'s idle block and `Server::reap`, an edge not worth a stand-in for a 15-minute unit. U83 spawns when U81 is on master. Housekeeping is aea639c14d8d038b7. Deployed: lbsim-00013-mvq from c22ea56; READY TO DEPLOY goes to main when U80–U83 are on master, again when U79 lands. **Done 73 · in flight 1 · queued 16 · waiting on Issao 1.**
...
flowchart TD
  ...
  U84[U84 Control panel header tag follows the mode]:::queued
  U85[U85 replay: load and policy tabs disabled, since every change is refused]:::queued
  U83 --> U84 & U85
  U87[U87 internal names out of the control panel and status bar]:::queued
  U88[U88 view-only sliders become readouts]:::queued
  U89[U89 Showcase and A/B leftovers: ellipsis, no-A/B-yet copy, empty pager, refusal checkbox]:::queued
  U83 --> U87 & U88 & U89
  U86[U86 first-sample race: serialised controls; poller past dispose; header word from first paint]:::flight
  U76 --> U86
  U79 --> U86
  U71 --> U80 & U81 & U82
  U81 --> U83
...
### U84 the Control panel header tag follows the mode (`model: sonnet`)
...
### U85 replay: the load and policy tabs are disabled, because every change is refused (decision, then `model: sonnet`)
...
### U87 internal names out of the control panel and status bar (`model: sonnet`)
...
### U88 view-only sliders become readouts (`model: sonnet`)
