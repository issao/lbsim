...
**Spawned 22:04** on `claude/tl-scheduling`, port 8208, as lane B. `SeqView`/`StepView`/`SchedulingPolicy { admit_order, prefill_budget, victim }`, views bounded to `max_batch` queue entries; `fifo_chunked` proven byte-identical before demo 16 is added; `class_priority`, `deadline_first`; key `scheduling`; arena rows flip; demo 16 under `slo_classes = interactive:0.7,agent:0.2,batch:0.1`; `ClassPriority class_priority = 14` is on master (d9a2757).
