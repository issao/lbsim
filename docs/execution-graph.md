...
**Resume:** branch `claude/tl-home-sections`, worktree `/home/agents/repo/lbsim-wt-home-sections`. Next step: finish Home.tsx per the brief, `(cd web && npm run build)`, `QA_PORT=8192 tools/qa/serve-local.sh` with reports, integrate.
...
**Resume:** branch `claude/tl-finished-text`, worktree `/home/agents/repo/lbsim-wt-finished-text`. Next step: the grep in the brief over index.html, Showcase.tsx (top function only), StatusBar.tsx, walkthroughs; reword; `npm run build`; `QA_PORT=8194 QA_SKIP_REPORTS=1 tools/qa/serve-local.sh`; integrate.
