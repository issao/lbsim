...
### U110 the export test cleans up its scratch (`model: sonnet`, tiny)
**Spawned 22:00** on `claude/tl-tmp-hygiene`. `tests/wire_export.rs`'s `fresh_dir` returns a guard that removes the directory on drop (kept on panic for inspection) and sweeps siblings older than an hour; measured cause of tonight's ENOSPC false "fingerprints moved".
